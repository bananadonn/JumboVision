...
export default function App(){
    ...
    useEffect(() => {
        ...
        const handleKeyDown = (e:KeyboardEvent) => {
            if (firstPress) {
              ...
              speak("JumboVision. An AI assistant for the visually impaired. Press right arrow key to learn how to use JumboVision.");
              firstPress = false;
            }
    ...
    const navBtnBase ="rounded-xl px-3 py-2 sm:px-4 sm:py-2.5 transition-all duration-200 whitespace-nowrap hover:bg-[#A157AB]";
    const navBtnFocus = "ring-2 ring-[#D4A843] ring-offset-2 ring-offset-[#3A114A] bg-[#76327D]";
    ...
    return(
        <div>
            <div className="font-bold gap-12 text-center text-4xl w-full bg-[#3A114A] py-4 px-6 flex items-center justify-center">
                {/* JUMBOVISION BUTTON */}

                <button className={`relative h-16 rounded-xl px-5 py-2.5 transition-all duration-300 hover:bg-[#A157AB]
                  ${focusedIndex === 0 ? "ring-2 ring-[#D4A843] ring-offset-2 ring-offset-[#3A114A] bg-[#76327D]" : ""}`}>
                    <div className="flex">
      ...
      <main className="mx-auto max-w-6xl px-4 sm:px-6 py-8 sm:py-12">
          ...
          {/* Video */}
          <div className="w-full max-w-md sm:max-w-lg item-cente">
            <video
              ref={videoRef}
              autoPlay
              playsInline
          ...
          {/* Start Camera */}
          <button
            className={`w-full max-w-sm rounded-xl px-5 py-5 text-base sm:text-3xl font-bold transition-all duration-200 ${
              focusedIndex === 4 ? "ring-2 ring-[#D4A843] ring-offset-2 ring-offset-black bg-[#76327D]" : "bg-[#3A114A] hover:bg-[#A157AB]"}`}
            onClick={async () => {
              // speak("Press the Start Camera button to begin.");
